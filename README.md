...
### v2.2 核心改进：运行时验证 + 干跑预览 + 渐进式成熟度

- **步骤自检**：每步结束强制对照 data-contract 验证必填字段产出
- **干跑预览**：中高风险任务先输出预览，用户确认后才执行
- **交叉审查**：REVIEW 后用独立视角做二次安全抽查
- **结构化日志**：JSONL 日志用于事后分析与持续优化
- **模型分层**：不同步骤推荐不同模型 tier，优化成本与质量
- **成熟度 L1/L2/L3**：团队可按阶段选装 4/7/10 个技能
- **依赖 DAG**：步骤间依赖关系可视化，条件分支自动触发
