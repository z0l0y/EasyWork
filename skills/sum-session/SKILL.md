...
description: >
  ...
  v2.12: 历史版本覆盖矩阵(铁律#36)+来源出处闸门(铁律#37)+证据账本(铁律#38)——版本→节点覆盖检查、每版本来源标注(摘要恢复须声明"非逐字原始记录")、每轮证据账本(最低样本数按风险等级)。
...
- ❌ v2.11：未 fetch 现有文档直接 overwrite——无基准覆写，C5 阻断（铁律 #29）
- ❌ v2.11：Quick Fix 时把整篇文档替换为 3 句话——round_report 冒充 engineering_active，C6+R3 阻断
- ❌ v2.11：本轮只改 5 行代码却重写整个文档——write_mode 错配，应用 quick_fix 仅追加版本记录，C7 阻断
- ❌ v2.11：Merge 后 READ/REVIEW/EXAMINE 节点消失——流程节点缩减，C4 阻断
- ❌ v2.11：新版本小节替代旧版本而非追加——历史版本丢失，C3 阻断
- ❌ v2.11：聊天摘要覆盖正式文档——round_report overwrite engineering_active，R4 阻断
- ❌ v2.11：已有文档用 overwrite 而非 structured merge——违反 R1，应默认 Normal
- ❌ v2.11：用户说"记录一下"不追问是 round_report 还是 engineering_active——作用域不明确
- ❌ v2.11：Overwrite 后不 fetch-compare——无法发现内容退化，违反 R6
- ❌ v2.11：以"本轮改动小"为由删减文档其他部分——Quick Fix 只能追加，不能缩减
- ❌ v2.11：跳过 Document Preservation Gate——detailed 模式此为 HARD GATE（铁律 #29）
- ❌ v2.12：版本覆盖缺失——版本索引有记录但对应节点下无独立 `### v{N}` 小节（铁律 #36）
- ❌ v2.12：版本仅有一行摘要无完整小节——不可仅用一行摘要代表整个版本（铁律 #36）
- ❌ v2.12：来源标注缺失——版本小节无来源类型标注(完整记录/摘要恢复/重构推断/外部引用)（铁律 #37）
- ❌ v2.12：摘要恢复未声明——使用摘要恢复但不声明"非逐字原始记录"（铁律 #37）
- ❌ v2.12：证据账本缺失——交付轮次无证据账本记录（铁律 #38）
- ❌ v2.12：证据样本数不足——证据数量低于风险等级对应的最低门槛（铁律 #38）

## 🆕 v2.8 文档迭代增量更新（铁律#25）— [已升级至 v2.10/v2.11]
...
>
> 详见下方 🆕 v2.10 三个新专区：Document Modes / Document Topology Gate / Structured Merge；以及 🆕 v2.11 三个新专区：Document Preservation Gate / Anti-Overwrite Rule / Write Mode Classification。
...
1. 加载 `skills/sum-session/references/anti-fluff-gate.md`
2. 获取待写入的完整报告内容
3. 按 6 类水文信号逐类扫描：
   - 类别1：空泛结论（关键词匹配）
...
见 `skills/sum-session/references/anti-fluff-gate.md` §4。
...
> 完整模式定义、结构模板、选择决策树、Mode A→B 转换规则详见 `skills/sum-session/references/document-modes.md`。
...
> 完整检测规则、解析算法、修复算法详见 `skills/sum-session/references/document-topology-gate.md`。
...
> 完整流程、版本号规则、变更类型标签、新文档初始化模板详见 `skills/sum-session/references/document-modes.md`。

---

## 🆕 v2.12 Historical Version Coverage — 历史版本覆盖（铁律 #36）

在 write_final_report 之前、Topology Gate 之后，必须检查版本覆盖完整性（见 `skills/sum-session/references/historical-version-coverage.md`）。

**检查内容**：版本索引→节点内版本小节覆盖矩阵。每个版本必须在对应影响节点下有独立的 `### v{N}` 小节，不可仅有一行摘要。

**Gate Check**：detailed→HARD GATE（有缺失节点→阻断写入，先修复覆盖），standard→SOFT GATE

## 🆕 v2.12 Source Provenance Gate — 来源出处闸门（铁律 #37）

每个版本内容必须标注来源类型（见 `skills/sum-session/references/source-provenance-gate.md`）。

**检查内容**：每个 `### v{N}` 小节标注来源——完整记录/摘要恢复/重构推断/外部引用。摘要恢复必须声明"该版本为摘要恢复，非逐字原始记录"。

**Gate Check**：detailed→HARD GATE（任何版本小节无来源标注→阻断；摘要恢复未声明→阻断），standard→SOFT GATE

## 🆕 v2.12 Evidence Ledger — 证据账本（铁律 #38）

每个交付轮次生成证据账本（见 `skills/sum-session/references/evidence-ledger.md`）。

**检查内容**：结论/证据类型(TEST_OUTPUT/CODE_DIFF等)/证据位置/可复现？。证据样本数≥最低门槛（L0:1/L1:2/L2:3/L3:4/L4:5）。

**Gate Check**：detailed→HARD GATE（证据数<最低门槛→阻断写入），standard→SOFT GATE

## 🆕 v2.11 Document Preservation Gate — 文档保真闸门（铁律 #29）

在 Document Topology Gate 通过后、Write 之前，必须检查内容保真度。加载 `skills/sum-session/references/document-preservation-gate.md`。

### 核心原则

**结构合格 ≠ 内容合格**。文档更新后字数/证据密度/历史版本数不得下降。对已有正式工程文档，默认使用局部更新，禁止无基准 overwrite。

### 七项内容保真检查

| # | 检查项 | 检测方法 | 判定 |
|---|--------|---------|------|
| C1 | 内容长度退化 | Fetch 现有文档→统计字数→merge 后统计→对比。merge 后 < 现有 80% → 阻断 | **HARD GATE** |
| C2 | 证据密度下降 | 统计代码摘录数/测试输出片段数/数据对比表数。任一指标下降 → 阻断 | **HARD GATE** |
| C3 | 历史版本丢失 | 统计 `### v{N}` 子节总数。数量下降 → 阻断。版本只增不减 | **HARD GATE** |
| C4 | 流程节点缩减 | 统计顶层流程节点数。节点数下降 → 阻断（Mode B） | **HARD GATE** |
| C5 | 无基准覆写 | 未先 Fetch 现有文档就写入 → 阻断。必须先读后写 | **HARD GATE** |
| C6 | round_report 冒充 engineering_active | 待写入 < 500 字 + 无代码摘录 + 无版本小节 → 阻断覆写，重定向为追加版本记录 | **HARD GATE** |
| C7 | 写入模式错配 | quick_fix 却 full overwrite → 阻断；normal 却只有摘要级内容 → 阻断 | **HARD GATE** |

### 判定规则

| 深度 | 闸门类型 | 命中处理 |
|------|---------|---------|
| **detailed** | **HARD GATE** | 拒绝写入 → 执行修复动作 → 重新检测（最多 2 轮） |
| **standard** | **SOFT GATE** | 展示退化指标 → 用户确认（默认修复） |
| **brief** | 跳过 | 标注 "⚠️ 简要摘要 — 内容保真未经检查，不适合工程追溯" |

> 完整检查规则、检测方法、修复策略详见 `skills/sum-session/references/document-preservation-gate.md`。

---

## 🆕 v2.11 Anti-Overwrite Rule — 禁止无基准覆写
...
| R3 | **Quick Fix 禁止覆写正式文档** | 小修小补只允许追加版本索引行 + 节点内新增版本小节，保留其他内容不变 |
| R4 | **round_report 不得 overwrite engineering_active** | 当轮摘要只能作为 round_report 独立记录或聊天回复，不得替换正式工程文档 |
| R5 | **Overwrite 仅限三种场景** | ①首次创建 ②用户明确要求重写整篇 ③已拿当前全文做完整 merge（fetch→merge→verify 内容未退化） |
| R6 | **Overwrite 后必须 fetch-compare** | overwrite 后立即 fetch 回读，对比字数/证据密度/版本数，发现退化→回滚+阻断 |

> 完整规则、适用条件、违规处理详见 `skills/sum-session/references/document-preservation-gate.md` §3。

---

## 🆕 v2.11 Write Mode Classification — 写入模式与文档作用域

### 写入模式三级分类

所有文档写入操作必须显式归类为以下三种之一：

| 模式 | 名称 | 适用场景 | 写入行为 | 可否 Overwrite |
|------|------|---------|---------|---------------|
| **Quick Fix** | 快速记录 | 微调(typo/config/样式)、<5行改动、无新增逻辑 | 仅追加：版本索引+1行 + 受影响节点下 `### v{N}` 小节 | **NEVER** |
| **Normal** | 正常更新 | 标准功能开发/Bug修复/重构（**DEFAULT**） | Full structured merge：Fetch→Parse→Pre-check→Distribute→Index→Mark→Verify | 仅限完整 merge 后 |
| **Full Archive** | 全量重建 | 首次创建/用户明确要求重写/已有全文完整 merge | 全量写入，附带 preservation checklist + post-write fetch-compare | 仅限上述三场景 |

**模式选择**：微调任务 → Quick Fix；新文档/已有文档+标准任务 → Normal（DEFAULT）；用户明确要求重写+已有全文 merge → Full Archive。

### 文档作用域

| 作用域 | 用途 | 可短？ | 可覆写正式文档？ |
|--------|------|--------|-----------------|
| **round_report** | 当前轮摘要——聊天回复、快速沟通 | YES (1-2 页) | **NEVER** |
| **engineering_active** | 长期工程档案——Code Review/交接/审计 | NO（保留全部历史） | 仅限 Full Archive 三合法场景 |

**混用阻断**：round_report 不得写入 engineering_active 路径。用户说"记录一下"→ Agent 必须追问：round_report 还是 engineering_active？

> 完整模式定义、决策树、行为规范详见 `skills/sum-session/references/document-preservation-gate.md` §4-§5。
...
0.2 **🆕 v2.11 Write Mode Classification**：确定 write_mode（quick_fix / normal / full_archive）→ 微调任务 → quick_fix，已有文档+标准任务 → normal（DEFAULT），用户要求重写+已 merge → full_archive
0.4 **🆕 v2.11 Document Scope Determination**：确定 document_scope（round_report / engineering_active）→ 默认 engineering_active。用户说"记录一下"必须追问
0.5 **🆕 v2.10 Structured Merge**（Mode B / normal 模式）：Fetch 现有文档 → 解析拓扑 → 分配本轮内容到节点
...
7.5 **🆕 v2.11 Document Preservation Gate（铁律 #29）**：加载 document-preservation-gate.md，7 项内容保真检查（C1-C7）+ Anti-Overwrite 规则（R1-R6）→ HARD/SOFT GATE 判定。命中→修复（补全内容/恢复版本/追加而非覆写）。Quick Fix 禁止覆写正式文档
7.6 **🆕 v2.12 Version Coverage Check（铁律 #36）**：加载 historical-version-coverage.md，检查版本索引→节点内版本小节覆盖矩阵。每个版本必须在对应影响节点下有独立的 `### v{N}` 小节。detailed→HARD GATE（有缺失节点→阻断写入，先修复覆盖），standard→SOFT GATE
7.7 **🆕 v2.12 Source Provenance Check（铁律 #37）**：加载 source-provenance-gate.md，检查每个版本小节是否标注来源类型（完整记录/摘要恢复/重构推断/外部引用）。摘要恢复必须声明"该版本为摘要恢复，非逐字原始记录"。detailed→HARD GATE（无来源标注或摘要恢复未声明→阻断），standard→SOFT GATE
7.8 **🆕 v2.12 Evidence Ledger Generation（铁律 #38）**：加载 evidence-ledger.md，生成证据账本。检查结论/证据类型/证据位置/可复现性。证据样本数≥最低门槛（L0:1/L1:2/L2:3/L3:4/L4:5）。detailed→HARD GATE（证据数<最低门槛→阻断写入），standard→SOFT GATE
7.9 **🆕 v2.12 Delivery Verification Checklist**：从 delivery_definition 生成交付验证清单，逐项确认交付物完整性（代码/测试/文档/Gotchas/配置/SQL）
...
10. **调用 write_final_report**：按后端定义的指令，将全部 10 步产出写入产物容器
    ...
    - 🆕 v2.11 传入：`document_scope`、`write_mode`、`preservation_gate_result`、`content_fidelity_snapshot`
11. **🆕 v2.9 Write-then-Fetch**：写入后 fetch 回读 → 验证完整性 → 修复循环（如需）
    - 🆕 v2.11 补充：如 write_mode=full_archive，额外执行 fetch-compare → 对比字数/证据数/版本数 → 发现退化→回滚+阻断
