...
name: knowledge-base
description: >
  Agent 知识库管理与沉淀（v3.0：SQLite + Stop Hook 每轮实时写入 + 配置驱动）。
  领域由 `.easywork/config.json` → `knowledge.domains` 动态定义，用户可自由增删改。
  区分来源（用户提供 inner / Agent 搜索 outer）和会话维度（用户提问 prompt / Agent 回答 output）。
...
capability:
  ...
  inputs:
    ...
    - { name: domain, type: enum, required: false, description: "领域：从 .easywork/config.json → knowledge.domains 动态读取（默认：backend|frontend|data|devops|mobile|design）" }
...
L2 — 领域知识（按需加载）
  └── knowledge/
      ├── domain/          ← 按项目需求领域组织（领域由 .easywork/config.json → knowledge.domains 动态定义）
      │   ├── backend/         ← 示例：后端开发（用户可自定义）
      │   ├── frontend/        ← 示例：前端开发（用户可自定义）
      │   └── ...              ← 更多领域按 config 动态创建
      ├── source/          ← 按来源类型组织
...
---
id: kb-20260627-001
domain: backend                  # 从 .easywork/config.json → knowledge.domains 中选取
...
## 2. 领域模型（Domain Model）—— v3.0 配置驱动

### 2.1 领域由用户配置

> 领域不再硬编码。用户在 `.easywork/config.json` → `knowledge.domains` 中自由定义。
> 每个领域包含：`id`（目录名）、`name`（中文名）、`emoji`（图标）、`keywords`（分类关键词）、`ttl_days`（知识过期天数）。

**内置默认领域**（可修改/删除/新增）：

| 领域 | ID | TTL | 典型粒度 | 示例 |
|------|-----|-----|---------|------|
| 💻 后端开发 | backend | 30天 | API/服务/数据库 | `POST /api/orders` 实现 |
| 🎨 前端开发 | frontend | 30天 | 组件/页面/交互 | 支付页重构 |
| 📊 数据工程 | data | 60天 | ETL/分析/报表 | 用户画像 pipeline |
| 🔧 DevOps | devops | 30天 | CI/CD/部署/监控 | K8s 集群迁移 |
| 📱 移动开发 | mobile | 30天 | App/小程序/跨端 | Flutter 支付模块 |
| 🎯 设计 | design | 90天 | UI/UX/设计系统 | 设计系统 v2.0 |

**分类决策**：Agent 根据当前任务涉及的文件路径和关键词，匹配各领域的 `keywords` 列表，自动确定 `domain`。

### 2.2 自定义领域结构

每个领域下的子目录结构由该领域的性质决定。短周期领域（如 API 联调）适合扁平结构，长周期领域（如 OKR/战略目标）适合层级结构：

```
knowledge/domain/{domain-id}/{feature-slug}/
├── _index.md            ← 该 Feature 的知识索引
├── requirements.md       ← 需求理解与完成定义
├── implementation.md     ← 变更记录与设计决策
├── review.md             ← 审查发现
├── retro.md              ← 复盘结论
├── summary.md            ← 六要素总结
└── progress.md           ← 进度跟踪（长周期领域专用）
```

**进度跟踪模板**（长周期领域如 OKR/战略目标）：

```markdown
---
domain: {domain-id}
status: in-progress
period: {YYYY-QX 或自定义周期}
objective: {O 的描述}
key_results:
  - KR1: {可量化指标} — 当前：{X}% — 目标：{Y}%
...
---
dimension: prompt
domain: backend                    # 从 config.json → knowledge.domains 中选取
...
|------|---------|--------|--------|-----------|
| 阅读整个模块的代码并分析架构 | ✅ 必须 | 按内容匹配 | derived | analysis |
| 用户提供了 API 文档让 Agent 理解 | ✅ 必须 | 按内容匹配 | inner | reference |
| Agent 搜索了 3+ 个外部资源做技术调研 | ✅ 必须 | 按内容匹配 | outer | reference |
| 用户问了一个复杂问题，Agent 长篇回答 | ✅ 应该 | 按内容匹配 | derived | output |
| 做了一个技术选型决策（A vs B） | ✅ 必须 | 按内容匹配 | derived | decision |
| 完成了一次功能开发/API 实现 | ✅ 必须 | 按内容匹配 | derived | analysis |
| 长周期目标进展更新 | ✅ 必须 | 按内容匹配 | derived | analysis |
| 用户简单问"这个函数是干嘛的" | ❌ 不捕获 | — | — | — |
| 改了一行配置 | ❌ 不捕获 | — | — | — |
| Agent 说了个笑话 | ❌ 不捕获 | — | — | — |
...
| api-test | API 规格、测试用例矩阵、错误码映射 | `domain/{匹配领域}/{api}/` | analysis |
...
- ❌ **长周期条目当普通条目处理**——长周期战略目标需要进度跟踪 + 阶段性总结 + 周期末复盘。不能像普通开发需求一样写一次就不管
...
Output: {
          "total": 15,
          "by_domain": { "backend": 8, "frontend": 5, "data": 2 },  // 示例——实际由 config.json 中的 domains 决定
          "stale_count": 3,
          "stale_entries": [...]
        }
```
...
- v3.0 (2026-06-29)：配置驱动重构——领域模型从硬编码三元组改为 `.easywork/config.json` 动态定义。6 个多角色默认领域（backend/frontend/data/devops/mobile/design）。知识库可配置开关（config.json → knowledge.enabled）。core vs utility 双层技能模型（skills/ vs tools/）。MCP Server 动态 domain enum + TTL 过期检查。
- v2.0 (2026-06-28)：SQLite + FTS5 存储架构——Stop hook 每轮实时写入 turns 表。MCP Server 增强。
...
- v1.0 (2026-06-27)：初始版本——三层渐进披露架构、三维领域模型、来源维度（inner/outer/derived）、会话维度（prompt/output）、五阶段操作流程（捕获/检索/存储/维护/交接）、ETR 证据链标准、14 条反模式
