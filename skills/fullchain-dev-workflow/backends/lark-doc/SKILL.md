...
name: lark-doc
description: >
  飞书文档后端适配器。将 EasyWork 9 步工作流产出生直接写入飞书文档，
...
**前置条件**：
- MCP `lark-mcp`（`@larksuiteoapi/lark-mcp`）已配置且可用
...
1. **检查 MCP 可用性**：`lark-mcp` MCP 服务是否已连接且正常响应
...
2. 调用 MCP 创建飞书文档：
   - 通过 `lark-mcp` 的工具（如 `create_document`）创建文档
...
2. 调用飞书 Markdown 转块 API 将内容追加到文档：
   - 通过 `lark-mcp` 的 `convert_markdown` 工具或等效调用
   - 追加到文档末尾
3. GIT 步骤特殊处理（见下）
4. 返回 `{ "success": true }`
...
2. 追加 SUM 六要素总结 + 跳过步骤汇总
3. 如有 TALK 产出，追加到 SUM 之后
4. 如有 ASK 确认结果，追加到最后
5. 追加文档尾注：
   ```
   ---
   *本文档由 EasyWork v2.5 自动生成，使用 lark-doc 后端。生成时间：{timestamp}*
   ```
6. 返回 `{ "success": true, "doc_url": "{飞书文档URL}" }`
