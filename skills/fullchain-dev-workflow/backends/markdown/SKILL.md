...
markdown 后端**不支持流式追加**（streaming: false）。每步骤产出在内存中保留**完整结构化数据**。

**🆕 v1.1 增量保障**：
- 每步骤完成后，Agent 将步骤产出**完整暂存**（按 data-contract 完整字段，不可精简为摘要）
- 最终 write_final_report 将所有暂存的完整步骤产出一次性写入 Markdown
- **禁止**：因上下文紧张而精简步骤产出
...
🆕 v1.6 新增输入：`risk_classification`（L0-L4风险等级）、`context_state`（状态文件路径）、`delivery_definition`（完成定义）、`evidence_ledger`（证据账本）
...
1. 按以下文档结构模板组织内容，**🆕 v1.1**：每个已执行步骤的完整产出必须填入
...
**任务**：{task_description}
**风险等级**：{risk_level}
**报告深度**：{report_depth}  ← 🆕 v1.1
**报告类型**：{report_type}  ← 🆕 v1.1
**产物后端**：markdown
**生成时间**：{timestamp}
**流式状态**：不支持（完整步骤详情在本文档中恢复）← 🆕 v1.1
