...
10. **变更前 checkpoint**：在执行任何会修改文件的步骤前（CODE、以及 CODE↔REVIEW 回退修复），若项目为 git 仓库，先确保工作区干净或已 stash。如果 REVIEW 回退达 3 轮上限 → 自动 `git stash pop` 恢复修改前的状态，挂起用户确认
...
| 5. GIT | git-split-commit | 按维度拆分提交 + Conventional Commits | 拆分方案 | 改动≤2文件且同维度 | Haiku / 快速 |
...
**Gotchas 追加自检**：Agent 在 SUM 步骤自检时，额外检查：如果本次工作流中遇到了 `references/gotchas.md` 定义的 4 种追加触发条件之一（耗时 >10min 定位的 bug / 反直觉陷阱 / 用户指出的边界 / 被反复问及的模式），是否已将新 Gotchas 追加到 `references/gotchas.md`。遗漏 → 补全后再进入 TALK。
