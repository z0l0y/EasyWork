...
本编排中枢集成了知识库（knowledge-base）作为跨切面能力。在以下步骤完成后，Agent 应自动将关键产出沉淀到 `knowledge/` 目录：

| 步骤 | 触发条件 | 沉淀内容 | 目标路径 |
|------|---------|---------|---------|
| READ | 阅读 ≥3 个文件 | 需求理解 + 完成定义 | `knowledge/domain/{domain}/` |
| CODE | 涉及 ≥2 个文件的改动 | 变更记录 + 设计决策 | `knowledge/domain/{domain}/` |
| REVIEW | 发现阻断问题或重要模式 | 审查发现 + 改进建议 | `knowledge/domain/{domain}/` |
| SUM | 总是 | 六要素总结（完整 ETR） | `knowledge/domain/{domain}/` |
| TALK | 总是 | 5-Whys 根因 + Trade-offs | `knowledge/domain/{domain}/` |
| 会话结束 | 总是 | 会话交接记录 | `knowledge/sessions/` |

沉淀策略：L0 分析 → 自动沉淀。L1+ 改动 → 展示摘要、用户确认后写入。详见 `skills/knowledge-base/SKILL.md`。
