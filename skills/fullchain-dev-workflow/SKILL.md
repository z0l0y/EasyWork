...
15. **仅在被显式调用时激活**：EasyWork 不是默认工作模式。Agent 仅在用户明确说"用 EasyWork"、"走 EasyWork 流程"、"EasyWork 模式"或类似显式调用时才启用完整 10 步流程。普通开发任务（用户说"修一下这个 bug"、"加个功能"、"帮我 review"）不自动套用 EasyWork 流程，除非用户明确要求。详见 `references/security-policy.md` §8
	...
	25. **文档迭代增量更新（🆕 v2.8）**：当对已有产物文档进行需求变更时，不可新建文档或新增顶层标题。必须在原有排版和标题结构下，以"更新记录"子节追加新内容，标注版本号和日期。同时检查已有内容是否有因本次变更而过时的信息，如有则标注 `[已过时 — v{X.X} 起]` 而非直接删除。确保文档可追溯完整演进历史，而非仅反映最新状态
...
| 7. SUM | sum-session | 按 report_depth 输出六要素 + MCR自检闸门 + 调用后端适配器写入最终报告 + **文档迭代增量更新**（铁律#25） | 总结文档 + 后端产物 | 极微改动且用户不需要记录（brief模式） | Sonnet / 标准 |
