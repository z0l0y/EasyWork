...
15. **仅在被显式调用时激活**：EasyWork 不是默认工作模式。Agent 仅在用户明确说"用 EasyWork"、"走 EasyWork 流程"、"EasyWork 模式"或类似显式调用时才启用完整 10 步流程。普通开发任务（用户说"修一下这个 bug"、"加个功能"、"帮我 review"）不自动套用 EasyWork 流程，除非用户明确要求。详见 `references/security-policy.md` §8
	...
	25. **文档迭代增量更新（🆕 v2.8，升级至 v2.10）**：当对已有产物文档进行需求变更时，不可新建文档。v2.8 方式：在文档末尾"更新记录"子节追加（仅 Mode A 审计日志下保留）。v2.10 方式（Mode B 持续维护，默认）：在对应流程节点内追加 `### v{N}` 版本小节，通过结构化合并（Fetch→Parse→Pre-check→Distribute→Index→Mark）实现。旧内容标注 `[已过时 — v{N} 起]` 而非删除。核心意图不变：确保文档可追溯完整演进历史
...
|------|---------|-----------|--------|---------|---------|
| 1. READ | read-requirements | 多态输入→结构化需求 + **需求理解确认**（铁律#24） | 五要素需求说明 + 澄清问题 | 用户指令极其精确，无歧义 | Haiku / 快速 |
...
| 4. EXAMINE | examine-quality | 找测试→跑→补→修→重跑至全绿 + **交互式应用真实体验验证**（铁律#26） | 质量报告+测试凭据 | 纯理解/纯文档，或项目无测试框架 | Sonnet / 标准 |
...
| 7. SUM | sum-session | 按 report_depth 输出六要素 + MCR+质量闸门 + Anti-Fluff Gate水文检测 + 同行审查就绪六问 + 质量评分 + **文档拓扑闸门(7项检查)** + 写后Fetch验证 + **禁止凭记忆写报告**（必须基于step_output拼装）+ **v2.10双模文档结构(审计日志/持续维护)+结构化合并** | 总结文档 + 后端产物 | 极微改动且用户不需要记录（brief模式） | Sonnet / 标准 |
...
- ❌ 需求变更时新建文档或新增顶层标题（🆕 v2.8）——破坏了文档的连续性和可追溯性。必须在原有排版下追加更新记录，标注版本号
