...
**并行优化**：GRAPH（步骤6）和 SUM（步骤7）在都只需 GIT 产出作为输入时，可以**并行执行**——它们之间没有依赖关系。Agent 在上下文充裕且平台支持并发时，可将这两步合并为并发阶段，减少总墙钟时间：
```
GIT 产出 → ┬→ GRAPH（图表绘制）
            └→ SUM（总结报告）
```
注意：如果上下文已处于 🟡 预警级别或更高，不要并行——两个步骤同时执行会加速上下文消耗。

**跨语言适配**：首次进入项目时，Agent 应先根据项目技术栈查阅 `references/language-matrix.md`，确定测试命令、风格工具、包管理器等命令的具体写法。后续步骤直接使用已确定的命令，不需要每次猜测。

**HTML 骨架复用**：生成 HTML 报告时，Agent 应直接复制 `assets/html-skeleton.html` 并替换 `{{PLACEHOLDER}}` 标记，而非从零生成 HTML。骨架已包含自适应 CSS 和步骤卡片结构。

**故障排查**：如果 Agent 在执行过程中遇到异常（测试命令找不到、上下文不足、回退循环超限等），应引导用户参考项目根目录下的 `TROUBLESHOOTING.md`。
