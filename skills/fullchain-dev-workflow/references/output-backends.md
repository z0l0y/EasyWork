...
capabilities: [create_doc, append_blocks, create_whiteboard, share_link]
...
| 约束 | 类型 | 说明 |
|------|------|------|
| `requires_mcp` | string[] | 依赖的 MCP 服务名（如 `lark-mcp`），空数组表示无依赖 |
...
| 依赖 | MCP: `lark-mcp`（`@larksuiteoapi/lark-mcp`） |
| 网络 | 需要 |
| 流式追加 | 支持（步骤完成即写入飞书文档） |
