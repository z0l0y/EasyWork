...
EasyWork 目前有 28 个技能节点，但它们是**孤岛**——每个只能单独调用，彼此不知道对方的存在。
...
| `needs_task_decomposition` | 需要需求拆解 | 复杂需求/技术方案需要拆分为可独立执行子任务/用户表达"不知道怎么开始"，建议调用 requirement-decompose 做六阶段拆解 |
...
| 🧩 requirement-decompose | development | requirement, context, granularity | task_map, completion_checklist, dependency_graph | read-requirements→, tech-compare→, →code-implement, →checklist, →learn-from-zero (六阶段拆解流水线——需求理解→纵向拆解→边界扫描→依赖映射→验收设计→完成清单) |
