...
- **Tab 补全**：输入 `/easywork:` 然后按 Tab，自动列出所有 19 个子命令
...
- ❌ **不要把 `disable-model-invocation` 设为 `false`**——19 个子命令如果全部注入 system prompt，每轮浪费 ~600 tokens。命令是给用户手动输入的，不需要模型自动调用。
...
- v1.0 (2026-06-27)：包含 19 个子命令（17 技能 + pipeline + meta），命名空间架构，命令索引表，维护流程，8 条反模式。
