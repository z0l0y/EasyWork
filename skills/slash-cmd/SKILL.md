...
name: slash-cmd
description: >
  Claude Code 斜杠命令调度与技能管理。将 19 个技能映射为 /easywork:<name> 命名空间子命令，
...
capability:
  ...
  related_skills:
    - { skill: all, relationship: outbound, desc: "为所有 19 个技能生成 /easywork:<name> 子命令入口" }
  suggested_when:
...
> 命令入口层 · v1.0 · 将 19 个技能映射为 `/easywork:<name>` 命名空间子命令
