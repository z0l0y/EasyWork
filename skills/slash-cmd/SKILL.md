...
> 命令入口层 · v3.0 · 将 25 个技能映射为 `/easywork:<name>` 命名空间子命令
...
### 技能发现范围：skills/ vs tools/

命令自动索引导入范围仅限于 `skills/` 目录，**显式排除 `tools/` 目录**：

| 目录 | 自动发现 | 生成斜杠命令 | 编排中枢可见 | 用途 |
|------|---------|-------------|------------|------|
| `skills/` | ✅ | ✅ | ✅ | 核心流程技能，质量受控，完整 frontmatter + references/ |
| `tools/` | ❌ | ❌ | ❌ | 对话生成的临时工具，手动 Read 调用，质量要求宽松 |

`tools/` 中的技能不注册为 `/easywork:*` 命令，不参与自然语言自动匹配，不纳入编排中枢的 DAG 节点。
