...
name: slash-cmd
description: >
  Claude Code 斜杠命令调度与技能管理。将 14 个技能映射为 /easywork:<name> 命名空间子命令，
...
capability:
  ...
  related_skills:
    - { skill: all, relationship: outbound, desc: "为所有 14 个技能生成 /easywork:<name> 子命令入口" }
  suggested_when:
...
> 命令入口层 · v1.0 · 将 14 个技能映射为 `/easywork:<name>` 命名空间子命令
...
- **Tab 补全**：输入 `/easywork:` 然后按 Tab，自动列出所有 16 个子命令
...
- ❌ **不要把 `disable-model-invocation` 设为 `false`**——16 个子命令如果全部注入 system prompt，每轮浪费 ~600 tokens。命令是给用户手动输入的，不需要模型自动调用。
...
- v1.0 (2026-06-26)：初始版本。16 个子命令（14 技能 + pipeline + meta），命名空间架构，命令索引表，维护流程，8 条反模式。
